...
"""
Adipose-LOGO RoPE Transformer (with optimal window search)

What's new:
- Leakage-safe per-shell optimal window search (train-only) via fast CV AUC.
- Replaces fixed Gen-3 windowing; you can constrain search with cfg.WIN_SEARCH_*.
- Keeps all your analysis, ECE, rollout, 5-run LOGO evaluation.

Outputs (unchanged):
- logo_runs_metrics.csv
- analysis/* (freq/heatmaps/features/rollout)

Update DATA_PATH and METADATA_PATH to your files.
"""

import os, math, pickle, warnings, csv, glob
import numpy as np
...
class CONFIG:
    ...
    # (Old fixed times removed from default use — can still be passed to force)
    ...
    # Output
    RESULTS_CSV: str = "logo_runs_metrics.csv"

    # Analysis outputs
    OUT_DIR: str = "analysis"
    N_HEATMAP_SAMPLES_PER_SHELL: int = 3

    # ---------- NEW: Window search controls ----------
    # Bin = one sample at 8 Gs/s => 0.125 ns per bin
...
def load_complex_data(path: str, is_pickle: bool) -> np.ndarray:
    if is_pickle:
        ...
        if path.endswith(".npy"): data = np.load(path, allow_pickle=False)
        elif path.endswith(".npz"): z = np.load(path, allow_pickle=False); data = z[list(z.files)[0]]
        else: raise ValueError("Use .npy/.npz for non-pickle.")
    arr = np.asarray(data)
...
def score_window_cv_auc(X: np.ndarray, y: np.ndarray, s_idx: int, e_idx: int,
    ...
    """
    Train-only scoring: 3-fold CV AUC on quick features from X[:, s:e, :].
    """
    Xw = X[:, s_idx:e_idx, :]
    ...
    for tr, va in skf.split(Xtr, ytr):
        ...
        try:
            ...
            # binary but rare class could break predict_proba in some edge solvers
...
def generate_window_grid(T: int,
    ...
    """
    Produce (s_idx, e_idx) pairs with inclusive start, exclusive end, stepping by 'step'.
    """
...
def compute_optimal_windows_per_shell(X: np.ndarray,
    ...
    """
    For each shell S (to be held out later), choose the best window using only train shells (!= S).
    We pick a single (s,e) per shell, re-used across all runs for reproducibility.
    """
    N, T, _ = X.shape
    ...
    # Use first run's seed for CV reproducibility in selection
    ...
    for S in shells:
        ...
        for (s_idx, e_idx) in grid:
            if e_idx - s_idx < 2:  # sanity
                continue
            auc = score_window_cv_auc(X, y, s_idx, e_idx, train_idx, n_splits=cfg.WIN_SEARCH_KFOLDS, seed=base_seed)
            if auc > best_auc:
                best_auc = auc
        ...
        print(f"[WIN] Shell {S}: best [{best_pair[0]}:{best_pair[1]}) len={best_pair[1]-best_pair[0]} "
              f"({start_ns:.3f}–{stop_ns:.3f} ns) CV-AUC={best_auc:.4f}")
    return by_shell
...
def split_aphant(phant_id: str) -> tuple[str, str]:
    if not isinstance(phant_id, str) or not phant_id.startswith("A"): return "UNK", "UNK"
    p = phant_id.find("F")
    if p <= 1: return "UNK", "UNK"
    return phant_id[:p], phant_id[p:]

# ========================== SANITIZERS ==========================

def finite_or_zero(a):
    out = np.array(a, copy=True); out[~np.isfinite(out)] = 0.0; return out

def finite_complex_or_zero(z):
    z = np.asarray(z); re, im = np.real(z), np.imag(z)
    mask = ~np.isfinite(re) | ~np.isfinite(im)
    if not mask.any(): return z
    zc = z.copy(); zc[mask] = 0.0 + 0.0j; return zc

# ========================== PATCH FEATURES (signal only) ==========================

def safe_stats(x):
    x = finite_or_zero(x); f={}
...
def prim_mst_total_length(points_2d):
    pts = finite_or_zero(points_2d); n=pts.shape[0]
    in_tree = np.zeros(n, dtype=bool); dist = np.full(n, np.inf); dist[0]=0.0; total=0.0
    for _ in range(n):
        u=int(np.argmin(dist))
        du=np.linalg.norm(pts-pts[u],axis=1); du=finite_or_zero(du)
        total += float(dist[u] if np.isfinite(dist[u]) else 0.0)
        in_tree[u]=True; dist[u]=np.inf
...
def topology_summaries(z72):
    ...
    return {"mst_len":mst_len,"cc_eps10":comps[0],"cc_eps20":comps[1],"cc_eps30":comps[2],"cc_eps40":comps[3],"cc_eps50":comps[4]}
...
def circ_fft_lowfreq(x,k=4):
    x = finite_or_zero(x)
    spec=np.fft.rfft(x,n=len(x)); mag=np.abs(spec)[1:k+1]
    return finite_or_zero(mag)

def phase_circular_stats(ang):
    ang = finite_or_zero(ang); C=np.mean(np.cos(ang)); S=np.mean(np.sin(ang))
    R=np.hypot(C,S); return {"phase_R":float(R),"phase_var":float(1-R)}

def zero_crossings(x): x=finite_or_zero(x); return float(np.sum(np.diff(np.signbit(x))!=0))
def autocorr_lag1(x): x=finite_or_zero(x); x=x-x.mean(); d=np.dot(x,x)+1e-12; return float(np.dot(x[:-1],x[1:])/d)
...
def extract_patch_features_one_timepoint(z_t_72: np.ndarray) -> Dict[str,float]:
    ...
    for name,arr in (("re",re),("im",im),("mag",mag),("ang",ang)):
        for k,v in safe_stats(arr).items(): f[f"{name}_k_{k}"]=v
    ...
    for name,arr in (("mag",mag),("ang",ang_unw),("re",re),("im",im)):
        ...
        for k,v in safe_stats(d1).items(): f[f"{name}_d1_{k}"]=v
        for k,v in safe_stats(d2).items(): f[f"{name}_d2_{k}"]=v
    for name,arr in (("mag",mag),("re",re),("im",im)):
        lf=circ_fft_lowfreq(arr,k=4)
        for i,v in enumerate(lf,1): f[f"{name}_fftk{i}"]=float(v)
    f.update(phase_circular_stats(ang))
    f["ang_unw_slope_var"]=float(np.var(np.diff(ang_unw)))
    for name,arr in (("re",re),("im",im),("mag",mag)):
        f[f"{name}_zcr"]=zero_crossings(arr); f[f"{name}_ac1"]=autocorr_lag1(arr)
    for k,v in list(f.items()):
        if not np.isfinite(v): f[k]=0.0
    return f

def build_signal_seq_features(Xw: np.ndarray) -> Tuple[np.ndarray, List[str]]:
    N,T,C = Xw.shape
    probe = extract_patch_features_one_timepoint(Xw[0,0]); keys = sorted(probe.keys())
    Fsig = len(keys)
...
def extract_safe_metadata(meta: List[dict]) -> Tuple[np.ndarray, dict, np.ndarray]:
    ...
    for i, m in enumerate(meta):
        ...
        for j, k in enumerate(SAFE_NUMERIC):
            v = m.get(k, 0.0)
            try: numX[i, j] = float(v) if v is not None else 0.0
            except: numX[i, j] = 0.0
        if cfg.INCLUDE_YEAR:
            yyyy = 0
            if isinstance(m.get("date",""), str) and len(m["date"]) >= 4:
                try: yyyy = int(m["date"][:4])
                except: yyyy = 0
            years[i] = yyyy
    meta_struct = {"F_comp": np.array(F_list, dtype=object),
...
class MHA_RoPE(nn.Module):
    def __init__(self, d_model, nhead, dropout=0.1, rope_max_len=512):
        super().__init__()
        ...
        self.rope = RotaryEmbedding(self.head_dim, rope_max_len)
...
class EncoderLayerRoPE(nn.Module):
    def __init__(self, d_model, nhead, d_ff, dropout, rope_max_len=512):
        super().__init__()
        self.norm1 = nn.LayerNorm(d_model)
        self.attn = MHA_RoPE(d_model, nhead, dropout, rope_max_len)
...
class EncoderRoPE(nn.Module):
    def __init__(self, n_layers, d_model, nhead, d_ff, dropout, rope_max_len=512):
        super().__init__()
        self.layers = nn.ModuleList([EncoderLayerRoPE(d_model, nhead, d_ff, dropout, rope_max_len) for _ in range(n_layers)])
...
class SeqClassifierRoPE(nn.Module):
    def __init__(self, in_dim, d_model, nhead, n_layers, d_ff, dropout, use_cls=True, num_classes=2, max_len=512):
        super().__init__()
        self.use_cls = use_cls
        self.proj = nn.Linear(in_dim, d_model)
        if use_cls:
            self.cls = nn.Parameter(torch.zeros(1,1,d_model)); nn.init.trunc_normal_(self.cls, std=0.02)
        self.enc = EncoderRoPE(n_layers, d_model, nhead, d_ff, dropout, rope_max_len=max_len)
    ...
    def forward(self, x, return_attn: bool=False):  # [B,T,F]
        ...
        h = self.proj(x)
        if self.use_cls:
            cls = self.cls.expand(B, -1, -1); h = torch.cat([cls, h], dim=1)  # [B,T+1,D]
        if return_attn:
...
def train_one_fold(tr_loader, va_loader, model, device, max_epochs, patience, lr, wd):
    opt = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=wd); crit = nn.CrossEntropyLoss()
    ...
    for ep in range(1, max_epochs+1):
        ...
        if va_loss + 1e-6 < best:
            best = va_loss; best_state = {k:v.cpu().clone() for k,v in model.state_dict().items()}; wait=0
        else:
            wait += 1
            if wait >= patience:
...
def eval_loader(loader, model, device, return_attn: bool=False):
    model.eval(); y_true=[]; y_pred=[]; y_prob=[]; rollout_all=[]
    with torch.no_grad():
        for xb,yb in loader:
            xb=xb.to(device)
            if return_attn:
...
def _freq_axis_labels(T, start_idx, f0_full, df):
    if f0_full is None or df is None:
        x = np.arange(T); xlabel = "Frequency bin"
    else:
...
def visualize_frequency_window_full_context(X, s_idx, e_idx, f0_full, df, outdir):
    ...
    ax.axvspan(x_full[s_idx], x_full[e_idx-1], color='grey', alpha=0.25, label=f'Window [{s_idx}:{e_idx}) len={win_len}')
    ax.set_xlabel(xlabel); ax.set_ylabel('Mean |Z| over N, antennas')
    ax.set_title('Global spectrum with window highlighted')
    ax.legend()
    fig.tight_layout(); fig.savefig(os.path.join(outdir, "freq_global_window.png"), dpi=200); plt.close(fig)
    ...
    fig.tight_layout(); fig.savefig(os.path.join(outdir, "freq_window_mean_abs.png"), dpi=200); plt.close(fig)
...
def heatmaps_per_shell_examples(Xw, A_comp, shells, outdir, per_shell=3):
    ...
    for s in shells:
        ...
        for k, i in enumerate(chosen, 1):
            M = mag[i].T  # (72,T) antennas x bins
...
def feature_stats_by_shell(seq_sig, A_comp, keys, outdir):
    ...
    ax.set_xlabel("ANOVA F-statistic"); ax.set_title("Top features by between-shell variance")
    fig.tight_layout(); fig.savefig(os.path.join(outdir, "top_features_by_ANOVA.png"), dpi=220); plt.close(fig)
    ...
    ax.set_xlabel("PC1"); ax.set_ylabel("PC2"); ax.set_title("PCA on per-sample mean features")
    ax.legend(frameon=True)
    fig.tight_layout(); fig.savefig(os.path.join(outdir, "pca_features_by_shell.png"), dpi=220); plt.close(fig)
    ...
    if len(sel)>0:
        ...
        for name in sel:
            ...
            ax.set_xlabel(f"Window bin (0..{T-1})"); ax.set_ylabel(name); ax.set_title(f"{name}: profile across bins (len={T})")
            ax.legend(frameon=True)
            fig.tight_layout(); fig.savefig(os.path.join(outdir, f"profile_bins_{name}.png"), dpi=220); plt.close(fig)

def aggregate_and_plot_rollout(attn_pattern="attention_*_rollout_*.npy", outdir="analysis"):
    _ensure_dir(outdir)
    ...
    any_shell = next(iter(shell_curves))
    V0 = np.vstack(shell_curves[any_shell])
    T = V0.shape[1]
    x = np.arange(T)

    for shell, li in shell_curves.items():
        V = np.vstack(li)
        mu = V.mean(axis=0); se = V.std(axis=0, ddof=1) / np.sqrt(max(V.shape[0],1))
        ax.plot(x, mu, label=shell)
        ax.fill_between(x, mu-se, mu+se, alpha=0.2)
        fig2, ax2 = plt.subplots(figsize=(7.2,3.2))
        ax2.plot(x, mu, lw=1.5); ax2.fill_between(x, mu-se, mu+se, alpha=0.25)
        ax2.set_xlabel(f"Token (0..{T-1})"); ax2.set_ylabel("Rollout importance"); ax2.set_title(f"Attention rollout — {shell} (len={T})")
        fig2.tight_layout(); fig2.savefig(os.path.join(outdir, f"rollout_profile_{shell}.png"), dpi=220); plt.close(fig2)

    ax.set_xlabel(f"Token (0..{T-1})"); ax.set_ylabel("Rollout importance")
    ax.set_title("Attention rollout (mean±SEM)")
    ax.legend(frameon=True)
    fig.tight_layout(); fig.savefig(os.path.join(outdir, "rollout_profiles_all.png"), dpi=220); plt.close(fig)
...
def build_meta_fit_transform(F_comp: np.ndarray, numX: np.ndarray, years: np.ndarray,
    ...
    ohe = _ohe_fit(); ohe.fit(F_comp[train_idx].reshape(-1,1))
    ...
    if cfg.INCLUDE_YEAR:
        year_ohe = _ohe_fit(); year_ohe.fit(years[train_idx].reshape(-1,1))
        year_feats = year_ohe.transform(years[apply_idx].reshape(-1,1)).astype(np.float32)
        mats.append(year_feats); meta_info["ohe_year"] = year_ohe
...
def run_one_shell_fold(shell: str,
    ...
    model = SeqClassifierRoPE(
        in_dim=in_dim, d_model=cfg.D_MODEL, nhead=cfg.N_HEAD,
        n_layers=cfg.NUM_LAYERS, d_ff=cfg.D_FF, dropout=cfg.DROPOUT,
        use_cls=cfg.USE_CLS, num_classes=2, max_len=(T+1 if cfg.USE_CLS else T)
    ).to(device)
    print(f"[{shell}] Model params: {sum(p.numel() for p in model.parameters())/1e6:.2f}M (in_dim={in_dim})")

    model = train_one_fold(train_loader, val_loader, model, device, cfg.MAX_EPOCHS, cfg.PATIENCE, cfg.LR, cfg.WEIGHT_DECAY)
...
def main():
    ...
    # --------- NEW: compute optimal (s,e) per shell using TRAIN-only CV ----------
    ...
    outdir = _ensure_dir(cfg.OUT_DIR)
    ...
    for run_idx, seed in enumerate(cfg.RUN_SEEDS, start=1):
        print(f"\n===== RUN {run_idx} (seed={seed}) — heads={cfg.N_HEAD}, layers={cfg.NUM_LAYERS} =====")
        set_seed(seed)
        per_shell = {}
        all_true = []; all_prob = []
        ...
        for shell in ADIPOSE_LIST:
            ...
            # (Optional) per-run one-time feature analysis (do once per run)
            if shell == ADIPOSE_LIST[0]:
                heatmaps_per_shell_examples(
                    Xw_shell, A_comp, ADIPOSE_LIST, outdir=os.path.join(outdir, "heatmaps"),
                    per_shell=cfg.N_HEATMAP_SAMPLES_PER_SHELL
                )
                feature_stats_by_shell(
                    seq_sig, A_comp, keys, outdir=os.path.join(outdir, "features")
                )

            m, y_true, y_prob = run_one_shell_fold(shell, seq_sig, y, A_comp, meta_struct, phant_ids, device, seed, save_rollout_prefix=save_prefix)
            per_shell[shell] = m
            all_true.append(y_true); all_prob.append(y_prob)
    ...
    # Mean ± std summary
    def mstd(x): x=np.array(x, float); return float(np.mean(x)), float(np.std(x, ddof=1)) if len(x)>1 else 0.0
    ...
    # Optional (run after training has produced attention_* files):
    # aggregate_and_plot_rollout(attn_pattern="attention_*_rollout_*.npy",
    #                            outdir=os.path.join(cfg.OUT_DIR, "rollout"))
